...
public final class var {
    ...
    public static void setData(String user, String pass) {
       try {
                ps=koneksi.prepareStatement("select * from users where id_user=AES_ENCRYPT(?,'nur') and password=AES_ENCRYPT(?,'windi') AND admin = 'true'");
                ps2=koneksi.prepareStatement("select * from users where id_user=AES_ENCRYPT(?,'nur') and password=AES_ENCRYPT(?,'windi')");
                try {
                    ps.setString(1,user);
                    ps.setString(2,pass);
                    rs=ps.executeQuery();
